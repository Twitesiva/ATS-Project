...
const EMPTY_FORM = {
  company_id: "", type: "Call", subject: "",
  notes: "", activity_datetime: new Date().toISOString().slice(0, 16),
};
  ...
  return (
    <div style={{ padding: "28px 32px", background: "#ffffff", minHeight: "100vh" }}>
      ...
      {showForm && (
        <div style={{ background: "#f8fafc", borderRadius: 12, padding: 24, marginBottom: 24, border: "1px solid #e2e8f0" }}>
          ...
          <Field label="Notes / Summary">
            <textarea style={{ ...inputStyle, height: 100, resize: "vertical" }} placeholder="Key discussion points, outcomes, next steps..." value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
          </Field>
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button onClick={() => setShowForm(false)} style={{ background: "#e2e8f0", color: "#0f172a", border: "none", borderRadius: 8, padding: "9px 20px", cursor: "pointer" }}>Cancel</button>
      ...
      {loading ? (
        ...
        <div style={{ background: "#f8fafc", borderRadius: 12, padding: 40, textAlign: "center", color: "#64748b", border: "1px solid #e2e8f0" }}>No communication logs found.</div>
      ) : (
              ...
              <div style={{ background: "#ffffff", borderRadius: 12, padding: "16px 20px", border: "1px solid #e2e8f0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div style={{ color: "#475569", fontSize: 11 }}>{new Date(log.activity_datetime).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</div>
                  </div>
                </div>
                <div style={{ color: "#475569", fontSize: 12, marginBottom: 6 }}>
                  🏢 {log.companies?.company_name || "—"}{log.companies?.contact_person ? ` · ${log.companies.contact_person}` : ""}
                </div>
