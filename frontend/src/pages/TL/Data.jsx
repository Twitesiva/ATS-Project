  ...
  const handleCSVUpload = async (e) => {
    ...
    const historyRows = [
      ...insertedRows.map((r) => buildHistoryRow({
        candidateId: String(r.id), recruiterName: r.recruiter || user?.name,
  ...
  const handleSave = async (form) => {
    ...
    if (insertedRow) {
      await insertStatusHistoryRows(
        buildHistoryRow({
          candidateId: String(insertedRow.id),
    ...
    <div style={styles.page}>
      ...
      <div style={styles.actionBar}>
        ...
        >
          ...
          <option value="hr round">Director Round</option>
...
function AddCandidateModal({ user, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (upsertedRow) {
      await insertStatusHistoryRows(
        buildHistoryRow({
          candidateId: String(upsertedRow.id),
  ...
  if (revError) {
    console.error("[status_update] revenue_tracker backout update failed", revError);
      ...
      const historyPayload = {
        candidate_id   : String(updated.id),
        recruiter_name : updated.recruiter || user?.name || "-",
        candidate_name : updated.candidate_name || "-",
        old_status     : record.status || null,
        new_status     : updated.status || form.status,
        updated_at     : new Date().toISOString(),
      };

      // Always INSERT a new history row for every status change (do not overwrite prior history).
      const { error: historyInsertError } = await supabase.from("status_history").insert([historyPayload]);
      if (historyInsertError) {
        console.error("[status_update] status_history insert failed", { historyInsertError, historyPayload });
        alert(`Status updated, but history log failed: ${historyInsertError.message || "Unknown error"}`);
      }
